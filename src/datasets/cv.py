...
def get_mnist_dataloader(batch_size=32, train=True):
    ...
    Args:
        ...
        train (bool, optional): If True, returns the training set.
            Otherwise, returns the test set. Defaults to True.
...
def get_imagenet_dataloader(batch_size=32, train=True):
    ...
    Args:
        batch_size (int, optional): Number of samples per batch. Defaults to 32.
        train (bool, optional): If True, returns the training set.
            Otherwise, returns the validation set. Defaults to True.
...
def get_coco_dataloader(batch_size=32, train=True):
    ...
    dataset = CocoDetection(
        root=f"data/coco/{split}",
        annFile=f"data/coco/annotations/instances_{split}.json",
        transform=transform,
    )
    return DataLoader(dataset, batch_size=batch_size, shuffle=True)
...
def get_ade20k_dataloader(batch_size=32, train=True):
    ...
    dataset = datasets.ADE20K(
        root="data", split=split, transform=transform, target_transform=transform
    )
    return DataLoader(dataset, batch_size=batch_size, shuffle=True)
